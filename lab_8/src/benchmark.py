...
def main():
    ...
    for test_case in test_cases:
        ...
        results[test_case['name']] = {
            ...
            'insertion': [],
            'search': [],
            'failed_search': []  # Add benchmark for failed binary search
        }
        ...
        for value in test_case['values']:
            ...
            results[test_case['name']]['search'].append(time_taken)

        for value in test_case['values']:
            time_taken = time_binary_search(tree,10000)  # A value guaranteed not to be in the tree
            results[test_case['name']]['failed_search'].append(time_taken)

    for operation in ['removal', 'insertion', 'search', 'failed_search']:  # Include failed binary search in plotting
